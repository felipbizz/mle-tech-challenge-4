...
def train_model(best_config) -> str:
    ...
    logger.info(
        f"Iniciando treinamento utilizandos os seguintes hiperparâmetros: {best_config}"
    )
    ...
    logger.info(
        f"Restringindo dados de treinamento a um ano. Tamanho do dataset: {len(train)}"
    )
    ...
    model_path = (
        f"ml_models/neuralforecast_lstm_{datetime.now().strftime('%Y%m%d_%H%M')}.joblib"
    )
